...
### 4.1 観測済みの環境依存ノイズ
ユーザー環境のブラウザー開発者ツールでは、以下のようなエラーが観測されている。
- Unchecked runtime.lastError: Could not establish connection. Receiving end does not exist.
- Uncaught (in promise) Error: Could not establish connection. Receiving end does not exist.
- msgport.js、content.js、all.js を起点とする拡張スクリプト由来の例外

これらは file 名と stack trace の形から、アプリ本体の Sources 配下ではなく、ブラウザー拡張または外部 injected script に起因する可能性が高い。

そのため、ランタイム不具合の調査では以下を明確に分離する。
- アプリ本体が送出した例外
- ブラウザー拡張が送出した例外
- ページに注入されたサードパーティスクリプトが送出した例外
...
### 5.3 テスト環境の前提
テスト実行時の正規環境は、ブラウザー拡張が無効なクリーンな実行コンテキストとする。

理由は以下。
- browser extension 由来の content.js、msgport.js、runtime.lastError はアプリ本体の品質評価を汚染する
- 拡張由来エラーをアプリ不具合として扱うと、原因切り分けが不正確になる
- Playwright のデフォルトコンテキストはこの観点で適している

したがって、テスト結果の評価基準は以下とする。
- Sources 配下のコードに起因する pageerror と console error は失敗とする
- 拡張由来の injected script エラーは参考情報として別枠管理する
- ユーザー環境でのみ再現し、Playwright のクリーン環境で再現しないものは、まず拡張干渉を疑う
...
- 拡張未導入のクリーン環境では runtime.lastError 系例外が発生しないこと
...
- 外部拡張由来の message port 失敗と、アプリ自身の Worker 通信失敗を混同しないこと
...
- ユーザー環境のブラウザー拡張に起因すると考えられる runtime.lastError / content.js / msgport.js 系エラー

ただし、3 点目は Playwright のクリーン環境では再現しておらず、現時点ではアプリ本体不具合ではなく環境依存ノイズとして扱う。

### 13.4 実施した修正
- Sources/js/worker-request-client.js に Worker 非対応時の graceful degradation を追加した
...
6. ユーザー環境で開発者ツールを確認する場合は、可能であれば拡張を無効化したブラウザープロファイルでも比較確認する
...
### 13.7 拡張由来エラーの取り扱い方針
以下の特徴を持つものは、第一段階ではブラウザー拡張由来の可能性が高いものとして切り分ける。
- stack trace が content.js、msgport.js、all.js を指す
- runtime.lastError、Could not establish connection. Receiving end does not exist. を含む
- Playwright のクリーン環境では再現しない

この場合の対応方針は以下。
- テスト計画には環境依存ノイズとして記録する
- アプリ本体の失敗判定には直ちに含めない
- 拡張無効環境でも再現する場合のみ、アプリ側の追加調査項目へ昇格する